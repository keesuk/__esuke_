...
export const ImgBox = styled.div`
    ...
    &:after {
        content: "";
        width: inherit;
        height: ${({height, hUnit, remarkHeight}) => 
            (parseInt(height)-parseInt(remarkHeight)) + hUnit
        };
        margin-top: -${({height, hUnit}) => parseInt(height) + hUnit};
        position: absolute;
    ...
    .remark {
        display: flex;
        line-height: ${({remarkHeight, hUnit}) => parseInt(remarkHeight) + hUnit};
        ${({remarkLorR}) => remarkLorR === "L" 
            ? null 
            : "flex-direction: row-reverse;"
        }
        margin: ${({remarkMargin}) => remarkMargin}; 

        .remarkEng {
            order: ${({remarkLorR}) => remarkLorR === "L" ? "1" : "2"};
            font-size: .5rem;
            font-family: 'IBM Plex Mono', monospace;
            font-weight: 500;
            font-style: italic;
        ...
        .remarkKor {
            order: ${({remarkLorR}) => remarkLorR === "L" ? "2" : "1"};
            font-size: .5rem;
