...
export const SeoulContents = {
    ...
    subText : {
        "0" : {
            ...
            "kor" : "서비스에 진입하게 되면 클릭을 유도하는 버튼과 말을 건네는 방식을 통해 다음 행동에 대한 힌트를 제시합니다."
        },
        "1" : {
            "title" : {
                "txtEng" : "Input",
            ...
            "eng" : "Users enter data about areas they know or are familiar with. For example, if the user is aware of the light, if the user clicks telephoto, the area that exists near the station is displayed as an image. The image appears in refined form rather than the familiar photograph. This is because refinement can lead to consensus about familiar areas from the user's point of view, along with whether or not to confirm that they know exactly from the data point of view. Click on the image to transform the ornaments. There are two reasons why the tournament is transformed. This is to induce fun through images that change unpredictably when clicking consecutively, and to provide image information suggesting the nature of the area to help you understand the vaguely known area.",
            "kor" : "사용자는 자신이 알거나 익숙한 지역에 대한 데이터를 입력합니다. 예를 들어, 사용자가 불광을 알고 있을 경우,  망원을 클릭하게 되면 역 근처에 존재하는 지역이 이미지로 표현됩니다. 해당 이미지는 흔히 알고 있는 사진이 아닌 정제된 형태로서 나타납니다. 정제는 데이터적 관점에서 정확히 알고 있는지에 대한 확인여부와 함께 사용자 관점에서 익숙한 지역에 대해 공감대를 이끌 수 있기 때문입니다. 이미지를 클릭하면 구성하는 오너먼트들을 변형시킵니다. 오너먼트가 변형되는 이유는 두가지가 존재합니다. 클릭이 연속으로 이루어질 때, 예측불가능하게 변경되는 이미지들을 통해 재미를 유도하고, 해당 지역에 대한 성격을 암시하는 이미지적인 정보를 제시하여 애매 모호하게 알고 있는 지역에 대한 이해를 돕기 위함입니다."
        },
    },
    bottomText :{
        "0" : {
