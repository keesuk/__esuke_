...
export const RulerContents = {
    ...
    mainText : {
        "0" : {
            "eng" : "There are things around us that we didn't know well because we were used to it. Dolls, cameras, bicycles, erasers, etc. are familiar, but we do not know deeply about these things. <br/>Magnified Ordinary Things is a group project that reinterprets the familiar but missed objects around us. Each of them composed content that could give an interesting interpretation of the given object, and edited 8 pages to match it. Among them, I approached the ruler, and also dealt with expandable objects such as protractors and figures connected to the ruler. The approach used the visual language of analysis to actively express the growing physical properties, and the logical expression method of disassembling and recombining it appropriate for this was mainly shown.",
            "kor" : "우리 주변에는 익숙하기 때문에 제대로 알지 못했던 사물들이 있다. 인형, 사진기, 자전거, 지우개등 익숙하지만 우리는 이들에 대해 깊게 알고 있지 않는다. <br/>‘확대해석 본’은 이처럼 익숙하지만 놓치고 있던 우리 주변의 물건들에 대해 다시 해석하는 단체 프로젝트이다. 각자 주어진 물건에 대해 재밌는 해석을 줄 수 있는 내용을 구성하고 그에 맞는 8페이지의 편집작업을 진행하였다. 그 중 나는 자에 대해 접근하였고, 더불어 자와 연결되는 각도기나 도형 등 같이 확장될 수 있는 물건들도 함께 다루었다. 접근은 자라는 물성을 적극적으로 표현하기 위해 분석이라는 시각적 언어를 활용하였고, 이에 걸맞는 분해하고 재조합하는 논리적인 표현방식이 주로 나타났다. ",
            "mark" : {
...
export const SeoulContents = {
    ...
    mainText : {
        ...
        "1" : {
            "eng" : "I thought that the logo should be able to present the characteristics of the service while creating a sense of sympathy for the various images of Seoul that individual citizens think of. To this end, two perspectives were applied. First, the elements of Seoul city (Han River, palace, existing logo, building) are depicted in the logo. Second, the individuality of each letter was emphasized by differentiating the elements (shape, ornament, and position) of each letter. <br/> Factors to be considered in supposing a key visual that will lead the flow of the entire service were that data visualization should be easy, and the design identity should be consistent. In setting the design identity, which is the second among them, circle, triangle, square, rhombus, and oval are tied in one common category of geometry, revealing the individuality of each ornament. It was appropriate to express this difference.",
            "kor" : "로고는 의미 그대로 시민 개개인이 생각하는 다양한 서울시에 대한 이미지의 공감이 일어남과 동시에, 서비스의 성격을 제시할 수 있어야 한다고 생각했습니다. 이를 위해, 두가지의 관점을 적용시켰습니다. 첫번째, 서울시 요소요소(한강, 고궁, 기존로고, 빌딩)를 로고에 묘사하였습니다. 두번째는 글자 개개의 구성요소(자형, 오너먼트, 위치)를 다르게 하여 개성에 대해 강조하였습니다. \n서비스 전체의 흐름을 이끌어 갈 키비주얼을 상정함에 있어 고려되어야 할 요소 첫번째는 데이터 시각화가 용이할 것, 두번째는 디자인적 정체성이 일관되어야 하는 것 이었습니다. 그 중 두번째인 디자인적인 정체성을 설정함에 있어, 원, 세모, 네모, 마름모, 타원은 오너먼트 각각의 개성이 들어나면서 기하도형이라는 하나의 공통 카테고리 안에 묶여 있는 점이 서울시민이라는 공통점을 지니면서 개개인의 개성이 다르다는 것의 표현이 적절하였습니다.",
            "mark" : {}
        },
    },
    subText : {
