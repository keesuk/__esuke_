...
import { GetScrollPosition, scrollTrigger } from "../../_data/_Functions";
    ...
    button {
        ${({theme, scroll}) => theme.mobile`
            ...
            -webkit-filter: ${scroll 
                ? "drop-shadow(-0.2rem 0.2rem 0.35rem rgba(0,0,0,0.22))" 
                : "drop-shadow(0rem 0rem 0.1rem rgba(0,0,0,0.22))"
            };
        `}
        ${({theme}) => theme.deskTop`
            padding: .6rem;
...
const Home = () => {
    return(
        <Link to="/">
            <HomeButton
                scroll={scrollTrigger(GetScrollPosition(), 200)}
            >
                <button>❋∵◇❋∵◇❋∵◇❋∵◇❋</button>
