...
import { line_1, line_3, rainbow, app_menu, app_aboutMe } from './_Functions.jsx';
...
class App extends Component {
    ...
    menu(){
        const menu = {}
        app_menu.map(v =>
            Object.assign(menu, {[v.title] : false})
        )
    ...
    aboutMe(){
        const aboutMe = {}
        app_aboutMe.map(v =>
            Object.assign(aboutMe, {[v.num] : { "value": v.num, "open": false }})    
        )
    ...
    postItHandler(v){
        ...
        if(v ==="AboutMe"){
            if(bool === false) this.aboutMe()
            else for(let key in aboutMeObj) aboutMeObj[key]["open"] = true
        }
        Object.assign(menuObj, {[v] : bool})
    ...
    postItReorder(num){
        const obj = this.state.aboutMe
        const last = Object.keys(obj).length-1

        for(let key in obj)obj[key]["value"] = obj[key]["value"]-1 

        obj[num]["value"] = last
        this.setState({aboutMe : obj})
    }
