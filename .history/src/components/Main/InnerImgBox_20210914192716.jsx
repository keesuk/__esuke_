...
const BrowserMockupDiv  = styled.div`
    width: ${({width}) => width}%;
    height: ${({width, VorH}) => VorH === "V" ? width : width/3}vw;
    margin-top: ${({marginTop}) => marginTop}vw;
    margin-left: ${({marginLeft}) => marginLeft}vw;
    margin-right: ${({marginRight}) => marginRight}vw;
    float: ${({LorR}) => LorR === "L" ? "left" : "right"};
    ...
    .box {
        width: ${({VorH}) => VorH === "V" ? "110" : "100"}%;
        height: ${({VorH}) => VorH === "V" ? "102" : "110"}%;
        background-image: url("${({VorH}) => VorH === "V" ? VerticalBrowserWhite : HorizontalBrowserDark}");
        background-position: center; 
        background-repeat: no-repeat;
        background-size: cover;
        .img {
            display: block;
            height: auto;
            width: ${({VorH}) => VorH === "V" ? "86.2" : "91.6"}%;
            float: right;
            margin-top: ${({VorH}) => VorH === "V" ? "9.1" : "5.7"}%;
            margin-right: ${({VorH}) => VorH === "V" ? "4.2" : "2.9"}%;
        }  
    }
`;
    ...
    return(
        ...
        >
            {contents.map((v, i) => {
                ...
                else if(v.type === "Browser"){Component = IphoneImg}
