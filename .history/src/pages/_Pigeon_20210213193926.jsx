...
const PageStyleObjDiv = styled.div`
    ...
    .notSticky {
        ...
        height: inherit;
        position: absolute;
        top: calc(${props => props.top}px + 1rem);
        left: 0;
...
const PageStyle = ({el, text, ord}) => {
    ...
    return(
        <PageStyleObjDiv 
            ...
            order={ord}
        >
            <div onClick={()=> ref.current.scrollIntoView({ behavior: "smooth"})}className={sticky ? "sticky" : "notSticky"}>{text}</div>
            <div ref={ref} className="notSticky"/>
        </PageStyleObjDiv>
)}
