...
const PageStyle = ({el, text, ord}) => {
    ...
    useEffect(() => {
        ...
        if(ref.current) { console.log('hi'); observer.observe(ref.current)}
        return () => {if(ref.current){ console.log('hi'); observer.unobserve(ref.current)} }

    }, [ref])
    
    return(
        <PageStyleObjDiv top={elTop}>
            <div ref={ref} className={sticky ? "sticky" : "notSticky"}>
                {text}
            </div>
        </PageStyleObjDiv>
)}
...
const _Pigeon = () => {
    ...
    return(<>
        ...
        <MainText>
            <EngContentsDiv ref={styRefs.current[0]}>
                <Marks>
