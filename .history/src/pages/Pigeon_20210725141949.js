...
const _Pigeon = () => {
    ...
    useEffect(() => {
        const scrollBottom = scrRef.current.scrollHeight - scrRef.current.scrollTop
        console.log(scrRef.current.scrollHeight, scrRef.current.scrollTop)
        scrRef.current.scrollToTop(0, -scrRef.current.scrollHeight)
    }, [])
    
    styRefs.current = pageStyle.map(i => styRefs.current[i] = createRef())

    return(
        <MainLayout ref={scrRef}>
            <MainTitleDiv
                color={color["sub"]}
                TorB={color["TorB"]}
            >
